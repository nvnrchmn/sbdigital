...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased text-slate-900 bg-slate-50 flex h-screen overflow-hidden" x-data="{ sidebarOpen: true, mobileMenuOpen: false }">
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden relative">
            ...
            @endphp
            <div class="sm:hidden fixed bottom-0 inset-x-0 bg-white border-t border-slate-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-40 px-6 py-2 pb-safe">
                <div class="flex justify-between items-center">
                    <!-- Home -->
                    <a href="{{ route('tenant.dashboard') }}" wire:navigate class="flex flex-col items-center p-2 {{ request()->routeIs('tenant.dashboard') ? 'text-indigo-600' : 'text-slate-500 hover:text-slate-900' }}">
                    ...
                    <!-- Iuran / Warga -->
                    @if($hasIuran)
                        <a href="{{ route('tenant.iuran') }}" wire:navigate class="flex flex-col items-center p-2 {{ request()->routeIs('tenant.iuran') ? 'text-indigo-600' : 'text-slate-500 hover:text-slate-900' }}">
                    ...
                    @else
                        <a href="{{ route('tenant.warga') }}" wire:navigate class="flex flex-col items-center p-2 {{ request()->routeIs('tenant.warga') ? 'text-indigo-600' : 'text-slate-500 hover:text-slate-900' }}">
                    ...
                    <!-- Menu / Lainnya -->
                    <button @click="mobileMenuOpen = true" class="flex flex-col items-center p-2 text-slate-500 hover:text-slate-900">
