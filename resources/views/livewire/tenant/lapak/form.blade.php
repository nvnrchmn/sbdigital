<div class="bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden w-full max-w-lg mx-auto flex flex-col max-h-[90vh]">
    ...
    <div class="p-6 overflow-y-auto">
        <form wire:submit="save" class="space-y-5">
            <div>
                <label for="nama_produk" class="block text-sm font-semibold text-slate-700 mb-1">Nama Produk / Jasa</label>
                <input wire:model="nama_produk" id="nama_produk" type="text" class="block w-full px-3 py-2.5 border border-slate-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-slate-50 focus:bg-white transition-colors" placeholder="Misal: Kue Nastar Kering, Jasa Servis AC" required>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input wire:model="harga" id="harga" type="number" min="0" class="block w-full px-3 py-2.5 border border-slate-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-slate-50 focus:bg-white transition-colors" placeholder="Misal: 50000" required>
            ...
            <div>
                ...
                <textarea wire:model="deskripsi" id="deskripsi" rows="3" class="block w-full px-3 py-2.5 border border-slate-300 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-slate-50 focus:bg-white transition-colors" placeholder="Jelaskan kondisi barang atau rincian jasa yang ditawarkan..."></textarea>
                <x-input-error :messages="$errors->get('deskripsi')" class="mt-1" />
            </div>

            <div>
                <label for="foto" class="block text-sm font-semibold text-slate-700 mb-1">Foto Produk (Opsional)</label>
                ...
                @if ($foto)
                    <div class="mt-2 mb-4 relative rounded-xl overflow-hidden border border-slate-200">
                        ...
                        <button type="button" wire:click="$set('foto', null)" class="absolute top-2 right-2 bg-red-600/80 backdrop-blur-sm text-white p-1 rounded-lg hover:bg-red-700 transition-colors">
                ...
                <input wire:model="foto" id="foto" type="file" accept="image/*" class="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 transition-colors cursor-pointer border border-slate-200 rounded-xl p-2 bg-slate-50">
                <p class="text-xs text-slate-500 mt-1.5">Format: JPG, PNG. Maksimal 2MB.</p>
                <x-input-error :messages="$errors->get('foto')" class="mt-1" />
                
                <div wire:loading wire:target="foto" class="text-xs text-indigo-600 mt-2 font-medium flex items-center gap-1.5">
