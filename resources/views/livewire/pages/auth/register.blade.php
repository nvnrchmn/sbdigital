    ...
    <form wire:submit="register" class="space-y-5">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input wire:model="name" id="name" type="text" required autofocus autocomplete="name"
                    class="block w-full pl-10 pr-4 py-3 border border-slate-200 rounded-xl bg-white focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200 sm:text-sm"
        ...
        @if (tenant())
            ...
            <div class="pt-4 pb-2 border-t border-slate-100">
                ...
                <div class="space-y-5">
                    <div>
                        <label for="nik" class="block text-sm font-medium text-slate-700 mb-1.5">NIK <span
                                class="text-red-500">*</span></label>
                        <input wire:model="nik" id="nik" type="text" required
                            class="block w-full px-4 py-3 border border-slate-200 rounded-xl bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200 sm:text-sm"
                            placeholder="Nomor Induk Kependudukan" />
                        <x-input-error :messages="$errors->get('nik')" class="mt-2 text-xs text-red-500" />
                    </div>

                    <div>
                        <label for="nomor_blok" class="block text-sm font-medium text-slate-700 mb-1.5">Nomor Blok /
                            Rumah <span class="text-red-500">*</span></label>
                        <input wire:model="nomor_blok" id="nomor_blok" type="text" required
                            class="block w-full px-4 py-3 border border-slate-200 rounded-xl bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200 sm:text-sm"
                            placeholder="Misal: Blok A1 No. 5" />
                        <x-input-error :messages="$errors->get('nomor_blok')" class="mt-2 text-xs text-red-500" />
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label for="status_warga" class="block text-sm font-medium text-slate-700 mb-1.5">Status
                                <span class="text-red-500">*</span></label>
                            <select wire:model="status_warga" id="status_warga"
                                class="block w-full px-4 py-3 border border-slate-200 rounded-xl bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200 sm:text-sm">
                        ...
                        <div>
                            ...
                            <input wire:model="no_hp" id="no_hp" type="text"
                                class="block w-full px-4 py-3 border border-slate-200 rounded-xl bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200 sm:text-sm"
                                placeholder="0812..." />
        ...
        <!-- Confirm Password -->
        <div>
            <label for="password_confirmation" class="block text-sm font-medium text-slate-700 mb-1.5">Konfirmasi
                Password</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <svg class="h-5 w-5 text-slate-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
        ...
        <div class="text-center text-sm text-slate-500 mt-6">
            Sudah punya akun?
            <a href="{{ tenant() ? route('tenant.login') : route('login') }}" wire:navigate
