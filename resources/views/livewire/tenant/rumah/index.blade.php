<div>
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
            <h2 class="text-3xl font-display font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-600 tracking-tight">Data Rumah</h2>
            ...
            <table class="w-full text-sm text-left whitespace-nowrap">
                ...
                <tbody class="divide-y divide-slate-100/60">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors duration-200 group">
                        ...
                        <td class="px-6 py-4">
                            <span class="text-slate-500 line-clamp-1 max-w-md">{{ $rumah->keterangan ?: '-' }}</span>
                        </td>
        ...
        <div class="md:hidden divide-y divide-slate-100/60 bg-white">
            ...
            <div x-data="{ expanded: false }" class="p-4 hover:bg-slate-50/50 transition-colors duration-200">
                ...
                <div @click="expanded = !expanded" class="flex justify-between items-center cursor-pointer">
                    <div class="flex items-center">
                        ...
                        <div class="ml-3">
                            ...
                            <div class="text-xs text-slate-500 mt-0.5 line-clamp-1">{{ $rumah->keterangan ?: 'Tidak ada keterangan' }}</div>
                        </div>
                    </div>
                ...
                <div x-show="expanded" style="display: none;" class="mt-4 pt-4 border-t border-slate-100">
                    <div class="mb-4">
                        <span class="block text-[10px] uppercase font-semibold text-slate-400 mb-1">Keterangan Detail</span>
                        <span class="text-sm text-slate-800">{{ $rumah->keterangan ?: '-' }}</span>
                    </div>
                    
                    <div class="flex justify-end gap-2">
                        @can('edit rumah')
                        <button wire:click="$dispatch('openModal', { component: 'tenant.rumah.form', arguments: { rumah: {{ $rumah->id }} } })" class="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 h-9 px-4 rounded-xl text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-100 transition-colors text-xs font-semibold">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
                            Edit
                        </button>
                        @endcan
                        @can('delete rumah')
                        <button wire:click="delete({{ $rumah->id }})" wire:confirm="Yakin ingin menghapus rumah ini?" class="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 h-9 px-4 rounded-xl text-red-700 bg-red-50 hover:bg-red-100 border border-red-100 transition-colors text-xs font-semibold">
