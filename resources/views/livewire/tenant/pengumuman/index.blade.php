<div>
    ...
    <div class="mb-8 relative w-full max-w-lg">
        ...
        <input wire:model.live="search" type="text" class="block w-full pl-11 pr-4 py-3 border border-slate-200/60 rounded-xl leading-5 bg-white/60 backdrop-blur-xl shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-shadow duration-200" placeholder="Cari judul atau isi pengumuman..." />
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <div class="bg-white/60 backdrop-blur-xl border border-white/40 shadow-sm hover:shadow-md transition-shadow duration-300 rounded-2xl p-6 flex flex-col relative group">
            ...
            <div class="flex items-center justify-between text-xs text-slate-500 mt-auto pt-4 border-t border-slate-100/80">
                ...
                <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                    @can('edit pengumuman')
                    <button wire:click="$dispatch('openModal', { component: 'tenant.pengumuman.form', arguments: { pengumuman: {{ $pengumuman->id }} } })" class="text-indigo-600 hover:text-indigo-800 transition-colors p-1" title="Edit">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
                    </button>
                    @endcan
                    @can('delete pengumuman')
                    <button wire:click="delete({{ $pengumuman->id }})" wire:confirm="Hapus pengumuman ini?" class="text-slate-400 hover:text-red-600 transition-colors p-1" title="Hapus">
