        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($allPlans as $p)
                <div class="bg-white rounded-2xl shadow-sm border {{ $plan && $plan->id == $p->id ? 'border-brand-indigo-500 ring-2 ring-brand-indigo-500/20' : 'border-slate-200' }} p-6 relative overflow-hidden flex flex-col h-full">
                    ...
                    @can('manage settings')
                        @if(!$plan || $plan->id != $p->id)
                            <button wire:click="subscribe({{ $p->id }})" wire:loading.attr="disabled" class="w-full bg-brand-indigo-600 hover:bg-brand-indigo-700 text-white font-semibold py-3 rounded-xl transition-colors shadow-sm mt-auto">
