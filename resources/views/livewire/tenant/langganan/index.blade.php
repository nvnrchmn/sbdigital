<div class="max-w-7xl mx-auto" @if($subscriptions->where('status', 'Pending')->count() > 0) wire:poll.3s @endif>
    ...
    <div class="mt-12 mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($allPlans as $p)
                <x-card class="{{ $plan && $plan->id == $p->id ? 'border-brand-indigo-500 ring-2 ring-brand-indigo-500/20' : '' }} p-6 relative flex flex-col h-full">
                    ...
                    @can('manage settings')
                        @if(!$plan || $plan->id != $p->id)
                            <x-primary-button type="button" wire:click="subscribe({{ $p->id }})" wire:loading.attr="disabled" class="w-full mt-auto justify-center">
    ...
    <!-- Riwayat Tagihan -->
    <x-card class="p-0 overflow-hidden mb-12">
